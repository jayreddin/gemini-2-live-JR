...
export class GeminiWebsocketClient extends EventEmitter {
    ...
    async sendJSON(json) {
        try {
            if (!this.ws) {
                console.warn(`[${this.name}] WebSocket is null. Cannot send message:`, json);
                // Optionally emit an event or attempt reconnect here
                return;
            }
            if (this.ws.readyState !== WebSocket.OPEN) {
                console.warn(`[${this.name}] WebSocket is not open (state: ${this.ws.readyState}). Cannot send message:`, json);
                // Optionally emit an event or attempt reconnect here
                return;
            }
            ...
            console.error(`[${this.name}] Failed to send message:`, json, error);
