...
export class GeminiAgent extends EventEmitter {
    ...
    setupEventListeners() {
        ...
        this.client.on('tool_calls', async (toolCalls) => {
            if (Array.isArray(toolCalls.functionCalls)) {
                for (const functionCall of toolCalls.functionCalls) {
                    const response = await this.toolManager.handleToolCall(functionCall);
                    await this.client.sendToolResponse(response);
                }
            } else {
                // Handle single tool call (if needed)
            }
        });
    }
    ...
    async initialize() {
        try {            
            ...
            // Check WebSocket connection before sending initial message
            if (this.client && this.client.ws && this.client.ws.readyState === WebSocket.OPEN) {
                // Only send the initial message if the connection is properly established
                try {
                    await this.client.sendText('.');  // Trigger the model to start speaking first
                } catch (error) {
                    console.warn('Failed to send initial message. Will continue without it:', error);
                    // Don't throw here - we don't want to fail initialization just because of this
                }
            } else {
                console.warn('WebSocket not open, skipping initial message. Will connect when needed.');
            }
        } catch (error) {
