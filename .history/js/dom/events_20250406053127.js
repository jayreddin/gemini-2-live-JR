...
const ensureAgentReady = async (agent) => {
    try {
        ...
        // Final check to ensure WebSocket is really ready
        if (!agent.client || !agent.client.ws || agent.client.ws.readyState !== WebSocket.OPEN) {
            console.warn('WebSocket not fully ready after connection/initialization. Trying to reconnect...');
        ...
        console.error('Failed to ensure agent is ready:', error);
        showConnectButton(); // Show connect button again if we failed
        throw error;
    }
};
